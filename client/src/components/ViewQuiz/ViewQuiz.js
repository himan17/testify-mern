...
class ViewQuiz extends React.Component {
    ...
    componentDidMount() {
        let id = this.props.param.get('id');
        this.setState({ id: id });
        this.refreshQuiz();
    }

    refreshQuiz = () => {
        axios.get('/api/quizzes/get-quiz/' + this.props.param.get('id')).then(res => {
            if (res.data) {
...
export default withRouter(ViewQuiz);